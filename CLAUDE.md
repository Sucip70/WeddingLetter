...
Semua fitur inti dari rancangan **sudah diimplementasikan dan diverifikasi**: 114 unit test API, smoke test alur penuh terhadap Postgres sungguhan (`apps/api/scripts/smoke.mjs`, 164 pengecekan: user + admin + job pemeliharaan), dan alur UI dicoba langsung di browser. Cara menjalankan ada di README.
...
- `templates/cover-layouts.ts` — tata letak sampul pilihan pembeli: `GENERIC_COVERS` (9, Standard & Premium) + `THEME_COVERS` (6 khusus tema, Premium; `THEME_COVER_BY_DESIGN`), `availableCoverLayouts(tier, motif)`. Disnapshot ke `layout.coverLayouts` lewat `withCoverLayouts()` dan dipilih pembeli di `data.cover.tata_letak` (FieldType `coverlayout`, divalidasi; Basic = tanpa pilihan). Bawaan per template = `layoutSchema.coverDefault` (builder: "Tata letak sampul bawaan"; kosong = otomatis khusus tema / Foto berbingkai; dibuang oleh `withCoverLayouts` bila tidak tersedia untuk paket/desain) dipakai editor, demo template, dan kartu katalog lewat `defaultCoverLayout(available, preferred)` di web.
...
- `components/invitation/motifs.ts` (paket motif per desain, kunci = `theme.motif`), `effects.tsx` (partikel, pola, efek sampul, bingkai foto, reveal saat gulir; partikel halaman Premium di undangan bergerbang baru dipasang saat sampul terlihat dengan `Particles enter` = masuk bertahap dari tepi layar, tanpa gerbang langsung penuh), `ornaments.tsx` (6 jenis ornamen), `gates.tsx` (21 gerbang pembuka Premium: adegan CSS/SVG, fase `closed`→`opening`→dilepas dikendalikan `InvitationView`; kunci = `theme.gate`, bawaan per desain di `DESIGN_GATES` API, hanya dipasang ke template Premium; tiga gerbang — `sakura` (kelopak), `frost` (kristal es), `leaves` (daun gugur, 1,5x lebih besar), semuanya `LetterGust` di `letter-gust.tsx` yang hanya beda rupa lewat `LOOKS` — memakai Motion (`motion/react`) alih-alih CSS murni, dipilih pembuat produk untuk koreografi bertahap yang sulit dilakukan lewat CSS keyframes; nilai acak kepingannya dibulatkan 3 desimal supaya SSR = hidrasi, hindari drift Math.sin antar mesin; gerbang ini menyingkap sampul selama fase `opening` lewat mask diagonal, jadi terdaftar di `REVEALS_COVER` dan `InvitationView` memutar animasi masuk sampul saat diketuk, bukan saat gerbang dilepas; `portal` juga memakai Motion lewat `MagicPortal` di `magic-portal.tsx`: lingkaran sihir tergambar lalu nama tamu "ditulis" per huruf, saat diketuk tersedot pusaran dan sampul tersingkap lewat lubang radial (`--hole`), juga di `REVEALS_COVER`; rupa `orbit` untuk motif `galaksi`, selain itu `rune`, dipilih dari prop `motif` yang diteruskan `InvitationView` ke `Gate`; `kayon` (bawaan Batik Jawa) = `WayangKayon` di `wayang-kayon.tsx`: kelir + blencong + gunungan bermotif kawung di gedebog; saat diketuk gunungan digoyang & dicabut, bayangannya (siluet SVG dengan lubang pintu gapura, `fillRule evenodd`) diperbesar bertahap geometris (`ZOOM`) sehingga sampul tersingkap lewat pintu, juga di `REVEALS_COVER`; nama tamu di portal & kayon ditulis per huruf lewat `inscription.tsx`; gerbang surat, portal & kayon memakai `STRINGS.id.guestFallback` bila `?to=` kosong). Menguji frame animasi Motion di browser pane yang tersembunyi: `delete Element.prototype.animate` + perlambat `performance.now` (Motion memakai jamnya sendiri; `getAnimations().pause()` saja tidak cukup). Animasi digerbangi `theme.fx`; `fx: none` harus tetap tampil seperti desain Basic lama. Pratinjau (editor/dashboard/builder) melewati gerbang (`gate="skip"`) dengan tombol putar ulang; halaman live dan demo template menampilkannya. Untuk memeriksa frame animasi di browser pane: patch `setTimeout` (1400-3000 ms), klik gerbang, lalu `getAnimations({subtree:true})` -> `pause()` + `currentTime`.
- `components/invitation/cover.tsx` (15 layout sampul berfoto; layout `ornamen`/bawaan tetap dirender `InvitationView`), `lib/cover-layouts.ts` (registri web, sinkron dengan API), `components/editor/cover-picker.tsx` (pemilih + ikon sketsa). Layout yang butuh foto (`needs`) kembali ke ornamen bila foto belum ada (`resolveCover`). Editor mengisi bawaan template (`coverDefault`, lalu khusus tema / Foto berbingkai) hanya bila draft belum punya pilihan.
