...
- **Template builder**: aktif/urut section, field wajib/label, desain (34 pilihan, per grup) + warna + font + level animasi, palet warna untuk pembeli, kuota foto/video, lagu khusus template (opsional; lagu bawaan datang dari pustaka musik), pratinjau langsung, publish/arsip.
...
- **Musik**: satu pustaka lagu di **Admin → Musik** (tabel `music_tracks`) melayani semua template. Tiap lagu punya paket minimum: "Semua paket" (mis. musik klasik domain publik), "Standard & Premium" (royalty-free dari library), atau "Premium saja". Pembeli hanya melihat lagu yang berhak dipakai paketnya; hasilnya tersimpan di snapshot undangan dengan rujukan `preset:<id>`. Lagu yang masih dipakai undangan tidak bisa dihapus permanen (arsipkan saja). Pustaka awalnya kosong: unggah sendiri, dan pastikan **rekamannya** (bukan hanya komposisinya) berstatus domain publik / CC0 / berlisensi komersial. Maks. 25 MB per lagu (lagu unggahan pembeli tetap 8 MB).
...
- **Tata letak sampul**: pembeli Standard & Premium memilih tampilan halaman pertama di editor (dan bisa mengubahnya kemudian di Dashboard → Isi undangan): 9 layout umum (ornamen, foto penuh, foto penuh nama di atas, foto berbingkai, jendela, medali bulat, terbagi, berdua, bingkai penuh) + 1 layout khusus tema untuk Premium (gapura, hati, portal, kristal, pilih karakter, poster film; pemetaan per desain di `apps/api/src/templates/cover-layouts.ts`, jaga sinkron dengan `apps/web/src/lib/cover-layouts.ts`). Pilihan disimpan di `data.cover.tata_letak` dan divalidasi terhadap `layout.coverLayouts`; foto sampul (`cover.foto`) terpisah dari galeri. Basic tanpa pilihan (ornamen, atau foto penuh bila foto sampul diunggah). Layout yang butuh foto kembali ke ornamen bila fotonya belum ada. Demo template menampilkan semua layout dengan foto demo (slot `cover`); setelah `git pull`, jalankan `npx prisma db seed` supaya slot `cover` terisi.
- **Foto demo**: demo template (halaman detail, beranda, pratinjau builder) memakai foto contoh dari tabel `demo_photos` (slot: mempelai pria/wanita, galeri 1-8, sampul) yang berkas-nya ada di folder `assets/` storage (R2). Foto bawaan adalah **ilustrasi placeholder** buatan kode (`apps/api/assets/demo/*.svg`, dibuat `node scripts/build-demo-photos.mjs`); ganti dengan foto asli di **Admin → Foto demo** (tombol "Bawaan" mengembalikan). `npm run demo:photos -w api` hanya mengunggah berkas bawaan ke storage (tanpa database); `npx prisma db seed` juga mendaftarkan slot yang masih kosong dan tidak menimpa foto yang sudah diganti. Halaman undangan pembeli tidak terpengaruh.
